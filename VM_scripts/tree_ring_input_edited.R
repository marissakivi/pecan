...
setwd("~/workflows/PEcAn_98000000048")

# desired species
species = c('QURU','ACRU','BEAL2','FAGR')

# unsure if we need these variables
var.names = 'AGB.pft'
...
spp_id <- match_species_id(species,format_name = 'usda',bety)
...
# load condensed data 
load('/data/dbfiles/met_data/HARVARD/condensed_HF.Rdata')

#input <- readRDS('/data/dbfiles/NPP_STAT_MODEL_HF.RDS')
# load condensed dataset from desktop

levels(dataset$taxon)[3] <- 'BEAL2'
#levels(dataset$taxon)[8] <- 'FRAM2'#7, removed in new condensing step 
