...
run.id = as.list(c(14000006582:14000006596))
time_to_rewind = 1961
...
sda_rewind <- function(settings,run.id,time_to_rewind){
  if(nchar(time_to_rewind) == 4){
    for(i in 1:length(run.id)){
      ...
      ### Remove netcdfs
      for(t in (as.numeric(time_to_rewind)):year(settings$state.data.assimilation$end.date)){
  ...
  files.last.sda <- PEcAn.ED2::list.files.nodir(file.path(settings$outdir,"SDA"))
